...
const updateForm =  (data: any, id: any) => {
  axios.put(environment.API_URL +`user/maintenance/update-location/${id}`,{
    id: data.id,
    name: data.name,
  }) .then((response) => {
...
<template>
  <VDialog v-model="isDialogVisible" max-width="600" top="true">
    ...
    <!-- Dialog Content -->
    <VCard title="Update Location / Work Place">
              ...
              <VCard flat>
                <VCardText>
                    ...
                    <VRow>
                      <!-- 👉 name -->
                      <VCol cols="12">
                        <VTextField v-model="data.name" :rules="[requiredValidator]" label="Location Name" />
                      </VCol>
