...
export const useCalendar = (event: Ref<Event | NewEvent>, isEventHandlerSidebarActive: Ref<boolean>, isLeftSidebarOpen: Ref<boolean>) => {
  ...
  const calendarsColor = {
    For_180_Days_Review: '#BAFF29',
    Birthday: 'success',
    Leave: 'error',
    For_Annual_Review: 'warning',
    For_90_Days_Review: 'info',
    Official_Business : '#46237A',
  }
  ...
  const extractEventDataFromEventApi = (eventApi: EventApi) => {
    ...
    const {
      ...
      url,
      extendedProps: { user_level, calendar,  location, description },
      allDay,
    }: Event = eventApi
  ...
  // If there's no info => Don't make useless API call
    ...
    store.fetchEvents()
      .then(response => {
        successCallback(response.data.events.map((e: Event) => ({
          ...e,
  ...
  const updateEventInCalendar = (updatedEventData: Event, propsToUpdate: (keyof Event)[], extendedPropsToUpdate: (keyof Event)[]) => {
  ...
  const updateEvent = (_event: Event) => {
    store.updateEvent(_event)
      .then(response => {
        const propsToUpdate = ['id', 'title', 'start', 'end', 'allDay'] as (keyof Event)[]
        const extendedPropsToUpdate = ['calendar', 'location', 'description', 'user_level'] as unknown as (keyof Event)[]
        updateEventInCalendar(response.data.updateEvent, propsToUpdate, extendedPropsToUpdate)
      })
  }
  ...
  */
  ...
  onMounted(() => {
    ...
    // console.log(calendarApi.value, "ON Mounted Here")
  })
