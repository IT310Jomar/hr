...
import DeclineSchedule from './DeclineSchedule.vue'
import ViewScheduleAccepted from './ViewScheduleAccepted.vue'
import ViewScheduleDeclined from './ViewScheduleDeclined.vue'
...
const props = defineProps(['paginatedDataPending', 'paginatedDataApproved', 'paginatedDataDeclined', 'editLeaverequest', 'viewLeaveRequestData',
    'handleFormUpdate', 'getScheduleRequestApproved', 'getScheduleRequestPending', 'getScheduleRequestDeclined', 'totalPages', 'totalPagesApproved', 'totalPagesDeclined',
...
const approvedScheduleData = ref()
const declinedScheduleData = ref()
const editScheduleData = ref()
const editRequestData = ref(false)
const declineRequestData = ref(false)
const viewAcceptedRequestData = ref(false)
const viewDeclinedRequestData = ref(false)
...
const declinedScheduleAdjustment = (data: any) => {
    declinedScheduleData.value = data
    declineRequestData.value = true
}

const EditScheduleAdjustment = (data: any) => {
    editScheduleData.value = data
    editRequestData.value = true
}

const viewAcceptedScheduleAdjustment = (Approved: any) => {
    approvedScheduleData.value = Approved
    viewAcceptedRequestData.value = true
}

const viewDeclinedScheduleAdjustment = (Declined: any) => {
    declinedScheduleData.value = Declined
    viewDeclinedRequestData.value = true
}

const requestDeclined = (id: any) => {
    axios.put(environment.API_URL + `user/schedule/adjustment/declined/schedulerequest/${id}`)
        .then((response) => {
            ...
            props.getScheduleRequestPending()
            props.getScheduleRequestDeclined()
        }).catch((error) => {
            toast("Cancel Error!", {
                autoClose: 4000,
                type: 'error',
...
const updateScheduleData = (viewScheduleAdjustmentData: any, id: any) => {
    axios.put(environment.API_URL + `user/schedule/adjustment/update/${id}`, {
        id: viewScheduleAdjustmentData.id,
        date_from: viewScheduleAdjustmentData.date_from,
        date_to: viewScheduleAdjustmentData.date_to,
        shift_start: viewScheduleAdjustmentData.shift_start,
        break_start: viewScheduleAdjustmentData.break_start,
        break_end: viewScheduleAdjustmentData.break_end,
        shift_end: viewScheduleAdjustmentData.shift_end
    }).then((response) => {
...
<template>
    <section>
        <v-card-title>My Schedule Adjustments</v-card-title>
        <VDivider />
                ...
                <VTable>
                    ...
                    <tbody>
                        <tr v-for="(data, index) in paginatedDataPending" :key="data.id" style="height: 3.75rem;">
                            ...
                            <td class="text-center">{{ moment(data.shift_start, 'hh:mm A').format('hh:mm A') }}</td>
                            <td class="text-center">{{ moment(data.shift_end, 'hh:mm A').format('hh:mm A') }}</td>
                            <td class="text-center">{{ moment(data.break_start, 'hh:mm A').format('hh:mm A') }}</td>
                            <td class="text-center">{{ moment(data.break_end, 'hh:mm A').format('hh:mm A') }}</td>
                            ...
                            <td class="text-center">
                                <VMenu v-for="menu in menusVariant" :key="menu">
                                    ...
                                    <VList>
                                        <v-list-item title="Cancel" @click="declinedScheduleAdjustment(data)" />
                                        <v-list-item title="Edit" @click="EditScheduleAdjustment(data)" />
                                    </VList>
                                </VMenu>
                            </td>
                        </tr>
                        ...
                        <tr v-for="(data, index) in paginatedDataApproved" :key="data.id" style="height: 3.75rem;">
                            ...
                            <td class="text-center">
                                <VMenu v-for="menu in menusVariant" :key="menu">
                                    ...
                                    <VList>
                                        <VListItem title="View" @click="viewAcceptedScheduleAdjustment(data)" />
                                    </VList>
                                </VMenu>
                            </td>
                        </tr>
                        ...
                        <tr v-for="(data, index) in paginatedDataDeclined" :key="data.id" style="height: 3.75rem;">
                            ...
                            <td class="text-center">
                                <VMenu v-for="menu in menusVariant" :key="menu">
                                    ...
                                    <VList>
                                        <VListItem title="View" @click="viewDeclinedScheduleAdjustment(data)" />
                                    </VList>
                                </VMenu>
                            </td>
                        </tr>
        ...
        <!-- Dialog For Declining Schedcule Adjustment Request -->
        <VDialog v-model="declineRequestData" max-width="600" persistent>
            <DeclineSchedule @closeDialog="declineRequestData = false" :declinedScheduleData="declinedScheduleData"
                :requestDeclined="requestDeclined" @form-canceled="handleFormCancel" />
        </VDialog>

        <!-- Dialog For Editing/Updating Schedule Adjustment Request Data -->
        <VDialog v-model="editRequestData" max-width="600" persistent>
            <EditSchedule @closeDialog="editRequestData = false" :editScheduleData="editScheduleData"
                :updateScheduleData="updateScheduleData" @form-submitted="handleFormSubmit" />
        </VDialog>

        <!-- Dialog For Viewing Accepted Schedule Adjustment Request Data -->
        <VDialog v-model="viewAcceptedRequestData" max-width="600" persistent>
            <ViewScheduleAccepted @closeDialog="viewAcceptedRequestData = false"
                :approvedScheduleData="approvedScheduleData" />
        </VDialog>
        <!-- Dialog For Viewing Declined Schedule Adjustment Request Data -->
        <VDialog v-model="viewDeclinedRequestData" max-width="600" persistent>
            <ViewScheduleDeclined @closeDialog="viewDeclinedRequestData = false"
                :declinedScheduleData="declinedScheduleData" />
        </VDialog>
    </section>
</template>
