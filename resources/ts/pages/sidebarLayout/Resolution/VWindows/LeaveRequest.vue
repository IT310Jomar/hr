...
const dleteLeave = ref()
const dialogVisible = ref(false);

const unresolveLeaveRequest = () => {
    ...
    .then(response => {
      if (response.data.success) {
        if (response.data.allData) {
          unresolves.value = response.data.allData[0].sick_vacation_leave.filter((leave : { status_id : string, date_from : any })=> leave.status_id === 'Pending' && leave.date_from < currentDate)
        }
      }
    })
}
...
const resolve = () => {
  ...
  .then(response => {
      if (response.data.success) {
        if (response.data.allData) {
          const approvedOvertime = response.data.allData[0].sick_vacation_leave.filter((leave: { status_id: string, date_from : any }) => leave.status_id === 'Approved' && (leave.date_from > currentDate || leave.date_from < currentDate))
...
<template>
  ...
  <VDialog v-model="dialogVisible" max-width="500">
    <VDialogDelete :dleteLeave="dleteLeave" :allData="allData" @toDeleteData="closeDialog" />
