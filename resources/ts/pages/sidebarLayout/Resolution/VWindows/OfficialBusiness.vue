...
const dleteOfficial = ref()
const dialogVisible = ref(false);

const unresolveOfficial = () => {
    ...
    .then(response => {
      if (response.data.success) {
        if (response.data.allData) {
          const pendingOvertime = response.data.allData[0].officialBusiness.filter((officialBusiness: { status_id: string, date_from : any }) => officialBusiness.status_id === 'Pending' && officialBusiness.date_from < currentDate)
...
const resolve = () => {
  ...
  .then(response => {
      if (response.data.success) {
        if (response.data.allData) {
          const approvedOvertime = response.data.allData[0].officialBusiness.filter((officialBusiness: { status_id: string, date_from : any }) => officialBusiness.status_id === 'Approved' && (officialBusiness.date_from > currentDate || officialBusiness.date_from < currentDate))
...
<template>
  ...
  <VDialog v-model="dialogVisible" max-width="500">
    <VDialogDelete :dleteOfficial="dleteOfficial" :allData="allData" @toDeleteData="closeDialog" />
