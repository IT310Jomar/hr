    ...
    .then(response => {
      if (response.data.success) {
        if (response.data.allData) {
          unresolves.value = response.data.allData[0].sched_adjustment.filter((sched : { status_id : string, date_from : any }) => sched.status_id === 'Pending' && sched.date_from < currentDate)
        }
      }
    })
}
...
const resolve = () => {
  ...
  .then(response => {
      if (response.data.success) {
        if (response.data.allData) {
          const approvedOvertime = response.data.allData[0].sched_adjustment.filter((sched: { status_id: string, date_from : any }) => sched.status_id === 'Approved' && (sched.date_from > currentDate || sched.date_from < currentDate))
...
<template>
  ...
  <VDialog v-model="dialogVisible" max-width="500">
    <VDialogDelete :dleteSched="dleteSched" :allData="allData" @toDeleteData="closeDialog" />
