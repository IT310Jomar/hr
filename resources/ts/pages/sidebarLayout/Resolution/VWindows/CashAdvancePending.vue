    ...
    .then(response => {
      if (response.data.success) {
        if (response.data.allData) {
          const pendingOvertime =  response.data.allData[0].cash_advancePending.filter((cash_advancePending: { request_status: string, requested_date : any }) => cash_advancePending.request_status === 'Pending' && cash_advancePending.requested_date < currentDate)
...
const resolve = () => {
  ...
  .then(response => {
      if (response.data.success) {
        if (response.data.allData) {
          const approvedOvertime = response.data.allData[0].cash_advancePending.filter((cash_advancePending: { request_status: string, requested_date : any }) => cash_advancePending.request_status === 'Approved' && (cash_advancePending.requested_date > currentDate || cash_advancePending.requested_date < currentDate))
...
watch(props, () => {
  if (!isEmpty(props)) {
    ...
    selectedTypes.value = props.inputs[0].types
    unresolve()
    resolve()
  } 
});
...
<template>
      ...
      <VTable class="px-5 mb-5">
        ...
        <tbody>
          <tr v-for="(user) in resolves" :key="user" style="height: 3.75rem;">
            <!-- 👉 User -->
            <td>{{  moment(user.date_approved).format('MMMM D, Y') }}</td>
            <td>{{ user.employee.first_name }} {{ user.employee.middle_name }} {{ user.employee.last_name }}
            </td>
  ...
  <VDialog v-model="dialogVisible" max-width="500">
    <VDialogDelete :dlete="dlete" :allData="allData" @toDeleteData="closeDialog" />
