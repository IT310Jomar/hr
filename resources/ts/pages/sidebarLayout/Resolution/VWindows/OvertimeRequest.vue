...
const dleteOvertime = ref()
const dialogVisible = ref(false);

const unresolveOvertime = () => {
    ...
    .then(response => {
      if (response.data.success) {
        if (response.data.allData) {
          const pendingOvertime = response.data.allData[0].overtime.filter((overtime: { status_id: string, shift_date : any }) => overtime.status_id === 'Pending' && overtime.shift_date < currentDate)
...
const resolve = () => {
  ...
  .then(response => {
      if (response.data.success) {
        if (response.data.allData) {
          const approvedOvertime = response.data.allData[0].overtime.filter((overtime: { status_id: string, shift_date : any  }) => overtime.status_id === 'Approved' && (overtime.shift_date < currentDate || overtime.shift_date > currentDate))
...
<template>
      ...
      <VTable class="px-5 mb-5">
        ...
        <tbody>
          <tr v-for="(user) in resolves" :key="user" style="height: 3.75rem;">
            <!-- 👉 User -->
            <td>{{ moment(user.updated_at).format('MMMM D, YYYY') }}</td>
  ...
  <VDialog v-model="dialogVisible" max-width="500">
    <VDialogDelete :dleteOvertime="dleteOvertime" :allData="allData" @toDeleteData="closeDialog" />
